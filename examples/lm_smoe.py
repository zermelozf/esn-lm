...
xtrain = esn_data(u1, y2, freservoir, initial_state, features, mode=mode)
xtest = esn_data(u2, y2, freservoir, initial_state, features, mode=mode)

input_dim, output_dim = xtrain.shape[1], y1.shape[1]
...
print ".. training"
m.fit(xtrain, ylabels)

K_soft_distrib1 = m.py_given_x(xtrain)
sim1 = np.mean(similarity(distrib1, K_soft_distrib1))
K_soft_distrib2 = m.py_given_x(xtest)
