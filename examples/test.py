...
features_dim, reservoir_dim = 2, 11
...
readout = MixtureOfExperts(input_dim=reservoir_dim, nb_experts=5, output_dim=output_dim)
...
import numpy as np
with open('./../datasets/xtrain') as f:
    xtrain = pickle.load(f)
with open('./../datasets/xtest') as f:
    xtest = pickle.load(f)
with open('./../datasets/ytrain') as f:
    ytrain = list(np.nonzero(pickle.load(f))[1])
with open('./../datasets/ytest') as f:
    ytest = list(np.nonzero(pickle.load(f))[1])

print "... training readout"
readout.fit(xtrain, ytrain, max_iter=15)
